...
input_shape = (561,) # get 1st sample's shape.

# Check shape of each partition. Each img is 32x32x3. 50000 in training set, 10000 in test set.
print("x_train shape = " + str(np.shape(x_train)))
print("y_train shape = " + str(np.shape(y_train)))
print("x_train shape = " + str(np.shape(x_test)))
print("y_train shape = " + str(np.shape(y_test)))
...
model_history = model.fit(x=np.array(x_train), y=np.array(y_train), epochs=EPOCHS, callbacks=[csv_logger, model_checkpoint], validation_data=(np.array(x_test), np.array(y_test)))
